...
import work.liy.mango.util.ChatUserInfo;
...
public class UserChatController {
    ...
    public List<ChatUserInfo> getAllChatUser(int userId) {
        ...
        for (MangoChatMessage mangoChatMessage : list1) {
            if (mangoChatMessage.getSender() == userId && !mangoChatMessage.getDelFlag().equals("1")) {
            ...
            } else if (mangoChatMessage.getReceiver() == userId && !mangoChatMessage.getDelFlag().equals("1")) {
    ...
    public R upload(@RequestParam("file") MultipartFile multipartFile, Integer source, String openId
        ...
        try {
            ...
            url = tencentCOSUtils.upload(multipartFile, tencentCOSUtils.getChatImg());
    ...
    // 删除聊天记录
    @PostMapping("/deleteChatUser/{userId}/{id}")
...
//        List<MangoScoreThemeVo> mangoScoreThemeVos = JsonUtils.jsonToList(mangoScoreItemPos, MangoScoreThemeVo.class);
        if(messageService.delete(userId, id)){
            return R.ok(200);
        }
        return R.error("删除错误");
    }
