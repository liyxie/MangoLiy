...
public class GetMessageDetailController {
    ...
    @PostMapping("/searchByKeyword/getMessageDetailByUserId/{userId}/{keyword}/{pageNumber}")
    public List<MangoMessage> searchByKeyword(@PathVariable String keyword,@PathVariable Integer userId, @PathVariable Integer pageNumber) {
        ...
        PageHelper.startPage(pageNumber, 5);
        PageInfo<MangoMessage> pageInfo = new PageInfo<MangoMessage>(mangoMessageDetailService.getMessageDetailByUserId(userId,keyword));
        List<MangoMessage> list = pageInfo.getList();
        getImage(list, mangoUserService, mangoMessageImagesService);
        if (pageInfo.getPageNum() < pageNumber) {
            List list1 = new LinkedList();
