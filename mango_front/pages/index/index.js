...
Page({
  data: {
    ...
    lost_new: {},
    takeout: [],
    ad_bottom: ["https://liy1900-1258789570.cos.ap-guangzhou.myqcloud.com/www/wwwroot/liy/images/other/ad_bottom.jpg"],
    user_message: [],
    activeIndex: 1,
  ...
  look_image(e) {
    ...
    if(this.data.swiper_images[index].url == null || this.data.swiper_images[index].url.length == 0){
      wx.previewImage({
        urls: [e.target.id],
      });
    }else{
      wx.navigateTo({
        url: "/pages/message_detail/message_detail?messageId=" + that.data.swiper_images[index].url
      })
    }
  ...
  getWeather() {
    ...
    //高德地图
    wx.request({
      url: 'https://restapi.amap.com/v3/weather/weatherInfo?city=440111&key=****************' ,
      method:'GET',
      success:(res)=>{
