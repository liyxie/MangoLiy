...
Page({
  ...
  comment() {
    ...
    if (that.data.comment_input.length<2) {
      wx.showModal({
        title: '提示',
        content: '至少输入2个字',
        showCancel: false
    ...
    wx.request({
      ...
      data: {
        ...
        "isAnonymousReply":that.data.isAnonymousReply
      },
      success: function(e) {
        wx.hideLoading()
        if (e.statusCode != 200) {
  ...
  // 匿名回复
  changeAnonymousReply(e){
    console.log("匿名开关："+e.detail)
    this.setData({
      isAnonymousReply: e.detail.value
    })
